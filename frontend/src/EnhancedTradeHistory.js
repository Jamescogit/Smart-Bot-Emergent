              ...
              sortedTrades.map((trade, idx) => (
                <tr key={trade._id || idx} className="border-b hover:bg-gray-50">
                  ...
                  {visibleColumns.exitReason && (
                    <td className="p-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        trade.exit_reason === 'TP Hit' ? 'bg-green-100 text-green-800' :
                        trade.exit_reason === 'SL Hit' ? 'bg-red-100 text-red-800' :
                        trade.exit_reason === 'Timeout' ? 'bg-yellow-100 text-yellow-800' :
                        trade.exit_reason === 'Manual Rule' ? 'bg-blue-100 text-blue-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {trade.exit_reason}
                      </span>
